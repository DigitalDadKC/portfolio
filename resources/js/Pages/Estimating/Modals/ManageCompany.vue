...
const handleFileInput = (e) => {
    form.logo = URL.createObjectURL(e.target.files[0])
}


const submit = () => {
    form.patch(route('companies.update', form.id), {
        onSuccess: () => {
            dialog.value = false
        }
    })
            ...
            <v-card prepend-icon="mdi-cog" :title="`Edit User: ${props.company.name}`">
                <v-card-text>
                    <v-row no-gutters>
                        ...
                        <v-col cols="12">
                            <v-img density="compact" label="Logo" height="255" :src="form.logo"></v-img>
                            <div v-if="$page.props.errors.logo" class="text-red-400">{{ $page.props.errors.logo }}</div>
                            <v-file-input show-size type="file" accept="image/*" label="Company Logo" @input="handleFileInput($event)"></v-file-input>
                        </v-col>
