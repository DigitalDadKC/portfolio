...
const biggest_job = ref(props.jobs[0])
const biggest_proposal = ref(biggest_job.value.proposals[0])
...
props.jobs.forEach(job => {
    job.proposals.forEach(proposal => {
        ...
        if(proposal_total > biggest_proposal.value.scopes.reduce((a, b) => a + b.lines.reduce((c, d) => c + (d.price*d.quantity), 0), 0)) {
