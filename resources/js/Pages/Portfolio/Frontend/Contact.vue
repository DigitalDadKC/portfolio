            ...
            <div class="flex flex-col lg:flex-row lg:gap-x-8 ">
                ...
                <form @submit.prevent="submit" class="space-y-4 w-full max-w-xl mx-auto">
                    ...
                    <div class="flex gap-8">
                        <div class="w-full">
                            <v-text-field v-model="form.name" hide-details label="Name" class="rounded-lg bg-light-tertiary dark:bg-dark-quatrenary text-black"></v-text-field>
                            <span v-if="form.errors.name" class="text-sm m-2 text-red-400">{{ form.errors.name }}</span>
                        </div>
                        <div class="w-full">
                            <v-text-field v-model="form.email" hide-details label="Email" class="rounded-lg bg-light-tertiary dark:bg-dark-quatrenary text-black"></v-text-field>
                            <span v-if="form.errors.email" class="text-sm m-2 text-red-400">{{ form.errors.email }}</span>
                        </div>
                    </div>
                    <v-textarea v-model="form.body" hide-details variant="outlined" label="Message" rows="5" row-height="18" class="mt-8 rounded-lg bg-light-tertiary dark:bg-dark-quatrenary text-black" auto-grow counter clearable=""></v-textarea>
                    <span v-if="form.errors.body" class="text-sm m-2 text-red-400">{{ form.errors.body }}</span>
                    <button class="btn btn-lg bg-accent hover:bg-secondary text-light-quatrenary dark:text-dark-quatrenary float-right rounded-md cursor-pointer" aria-label="Submit Message">
                        Send Message
                    </button>
                </form>
            </div>
